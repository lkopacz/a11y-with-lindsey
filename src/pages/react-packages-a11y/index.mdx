...
This blog post writes under the assumption that you are familiar with the basic terms of React and understand the syntax of JSX. If you are not familiar with it, I highly suggest you get familiar with it, I suggest that you read through the following resources:
...
I use CodeSandbox for all of my examples, so if you'd like to learn by doing and don't feel like setting up your environment, I highly recommend you Codesandbox. If you'd like you can fork my examples, play with the styling, and manually test the components.
...
1. The hamburger button has clear focus
...
1. In `src` directory, create a component called `BurgerMenu.js`. Import the desired Menu animation (I used `slide`) and use that component to add links to your menu.
...
_NOTE: there are many different animation styles you can use. I chose slide for this example, but you can see how all the options animate in the [demo](https://negomi.github.io/react-burger-menu/). Switch out the `slide` for whichever animation you'd like, spelled out in the [docs](https://github.com/negomi/react-burger-menu#animations)_

1. Add it to the `App.js` component
...
1. Create a CSS file and import it into your react component. Add some CSS to position some of the elements, give background colors to the close cross button and the hamburger bars, etc.
...
Do some manual testing because you never know even if it seems ok.

1. Go through on a keyboard 👍🏼 TODO ADD VIDEO WITH KEYBOARD.
1. Go through on a VoiceOver (learn that you can still hear all the links even though you can't tab to them)
1. Voiceover goes to menu items that are hidden, we need to add aria-hidden and toggle it. We'll use `useEffect` on the parent component to add that attribute when the component has mounted.
...
- After reading the [docs](https://github.com/negomi/react-burger-menu#state-change), I saw `onStateChange` so we are going to use that to toggle the aria-hidden attribute dependent on the isOpen state.
...
refactor with destructuring the state (add a link to explain desctructuring in the parameters)
...
then use the state to control the aria hidden
...
Now we want to make sure that everything is working how we would expect with both a screen reader and a keyboard.

<!-- Add a few things about the differences between adding the UL versus just the links -->
...
1. 1. In `src` directory, create a component called `Modal.js`. Import `ReactModal` and create the base of your function component.
...
1. Attach the Modal to the root of your app (this toggles aria-hidden on the root when it's open)
...
1. add `useState` hook to the Modal component and use the hook to destructure an `isOpen` variable and `setIsOpen` function. The initial state of the modal being open is `false` (because we don't want to start with an open modal)
...
(add link about react fragments)
...
Something I am going to do to make this Accordion more reusable, is pass all the data into the component using a prop. Then we can map through the data and make our code cleaner.
